...
test('unsupported paths remain denied and recommendation actions stay keyboard-sized', async () => {
  ...
  assert.match(component, /onClick=\{\(\) => selectTab\('decision-lab'\)\}/)
  assert.match(component, />Open Decision Lab<\/button>/)
  assert.doesNotMatch(component, /Paper decision — later gate/)
