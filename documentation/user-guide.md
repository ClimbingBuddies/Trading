...
2. **Recommendations** is your default view, also available at `/watchlists`. Choose your list, add shares by ticker or company, and see the latest saved scheduled AI assessment alongside its matching published plan. Filter **All watched shares** or **AI buy candidates**, and switch Weekly (5 sessions) / Monthly (20 sessions). Research older than 72 hours is labelled; missing assessments and plans are never fabricated. For fresh eligible evidence, AI chooses the entry session and holding period separately for each outlook. Session counting begins after publication; the original plan is locked. Older fixed-rule records remain labelled. Stale research, missing daily history or missing matching-currency benchmarks prevent new timing forecasts. Use **Enable tracking** if tracking is off, and open **Decision Lab** to review outcomes.
