...
| `/my-dashboard` | Private personal workspace. Recommendations shows append-only AI calls with fixed 5/20-session entry/exit plans; Decision Lab automatically tracks these same records and compares completed buy outcomes with the named benchmark. Both use the current authenticated owner independently of legacy portfolio loading. The other tabs retain their existing implementation. | `personal_prediction_tracking`, `personal_prediction_plans`, `personal_prediction_results`, constrained authenticated enrolment RPC; existing owner tables for other tabs | Permanent authenticated owner; no privileged credentials in the browser |
