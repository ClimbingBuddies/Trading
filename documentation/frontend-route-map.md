...
|---|---|---|---|
| `/` | Redirect to sign-in with My Dashboard as the safe post-authentication destination | None | Public |
| `/login` | Supabase Google and email/password authentication; accepts only a safe local `next` destination and defaults to My Dashboard | Supabase Auth | Public authentication boundary; authenticated sessions continue to their safe destination |
| `/my-dashboard` | Private six-tab personal research shell; Today, preferences, Watchlists, relevant Opportunities, Portfolio Health and Recommendations use persisted owner data. Recommendations read immutable snapshots, separated provenance families and append-only events, displaying risks, quality limitations, cutoffs and methodology without a Buy/Sell path. Portfolio Health supports private portfolio headers, validated manual position entry and owner-scoped persisted health snapshots with explicit loading, empty, incomplete, error and provenance states. Trusted calculations never run in the browser. | `user_market_preferences`, `user_market_interests`, private `watchlists` / `watchlist_items`, independent Opportunity mappings and assessments, owner-scoped portfolio and personal-recommendation tables, canonical `instruments`, constrained authenticated event/health functions | Permanent authenticated owner; anonymous and signed-out sessions receive no personal rows and are routed to sign-in |
