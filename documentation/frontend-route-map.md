...
| `/my-dashboard` | Private six-tab personal research shell; Today, preferences, Watchlists, relevant Opportunities, Portfolio Health, Recommendations and Decision Lab use persisted owner data. Decision Lab separates AI and user clocks, reads immutable decisions/events, and offers AI capture only from database-reported forward-eligible assessment IDs; the capture RPC remains authoritative and idempotent. Recommendations display separated provenance, risks, quality limits, cutoffs and methodology without a Buy/Sell path. Trusted calculations never run in the browser. | `user_market_preferences`, `user_market_interests`, private `watchlists` / `watchlist_items`, independent Opportunity mappings and assessments, owner-scoped portfolio/recommendation/decision tables, canonical `instruments`, constrained authenticated event/health/decision functions | Permanent authenticated owner; anonymous and signed-out sessions receive no personal rows and are routed to sign-in |
