...
The status, remaining scope and run forecast are kept together here so gate progression can be reviewed side by side. Update the applicable row after every gate transition or material scope change.

| ID | Current status | Gate | Definition of done | Outstanding work / next progression | Estimated remaining runs |
|---|---|---|---|---|---:|
| MYDASH-001 | DONE | Product, data and calculation contract | Reconcile existing tables and Strategy Laboratory reuse; produce the final route/tab contract, exact data dictionary, ownership model, RLS test matrix, recommendation provenance contract, portfolio-health definitions, decision clocks, return formulae, benchmark/FX/corporate-action assumptions and migration plan. No production schema or UI changes. Independent audit required, then Owner Review A. | Complete; independently audited and Owner Review A accepted. | 0 |
...
| MYDASH-004 | IN_PROGRESS | Portfolio and health | Manual portfolio/position management, deterministic health measures, immutable snapshots and authenticated UI are implemented in the current candidate; its trusted evaluator fully pages cutoff-effective issuer mappings with a unique mapping-ID tie-breaker, rejects ambiguity, and independently enforces applicability, lowercase canonical UUID issuer identity, canonical non-empty source/methodology labels, already-trimmed query- and fragment-free credential-free HTTPS source URLs, exact calendar-date validity, chronology and cutoff-effective validity before immutable hashing. The undeployed issuer schema independently enforces UUID issuer identity plus the same trimmed label and URL boundaries. Every valid issuer field remains bound to source identity. The Windows development command now selects the documented Webpack fallback and has fresh full-suite and authenticated localhost verification. The required Twelve Data ASX entitlement evidence remains absent; the issuer schema remains undeployed/unseeded. | Obtain the required non-secret owner evidence before the approved Twelve Data reference lookup; review any daily-source methodology change before mapping or persistence; deploy/verify and seed the issuer contract only in a separately authorised cycle; complete hosted same-cutoff replay and Test A versus owner cross-user confinement; then complete independent audit and Owner Review B. | 2–4 |
| MYDASH-005 | PLANNED | Explainable Recommendations | Implement immutable personal recommendation snapshots and Recommendations UI. Prove source lineage, cutoff dates, methodology separation, risk display, dismissal/feedback behaviour and no unsupported recommendation paths. | Build explainable immutable recommendations, provenance/risk presentation, feedback boundaries, tests and independent audit. | 5–8 |
| MYDASH-006 | PLANNED | Decision Lab capture | Implement separate AI-signal and user-paper decision capture, immutable snapshots, entry-resolution states, horizons, decision events and open/completed views. Reuse Strategy Laboratory structures only where the MYDASH-001 contract proved semantic compatibility. | Build immutable AI-signal and user-paper decision capture, events, entry-resolution states, UI, tests and independent audit. | 5–8 |
...
**Forecast updated:** 5 September 2026, 17:39 Australia/Perth

**Current position:** The trusted issuer evidence boundary now requires the lowercase canonical UUID representation emitted by the database, closing a mismatch that previously allowed malformed or case-only issuer identity variants into immutable hashing; focused regression passes. The 5 September local-development recovery remains valid and the authenticated Portfolio Health localhost panel remains the browser-verification target. The operational access/server blocker remains cleared and failure continuity is `0 / NONE`. No new working-authority evidence exists for the configured Twelve Data ASX add-on, completed exchange registration or public-display redistribution rights, so the owner-approved reference lookup remains uncalled. MYDASH-001 through MYDASH-003 are complete; MYDASH-004 is in progress. All 15 active canonical ASX instruments remain unsupported by Tiingo. The issuer schema remains undeployed/unseeded; hosted same-cutoff replay and Test A versus owner confinement remain outstanding. The cross-gate forecast remains 23–39 bounded runs, with MYDASH-004 at 2–4. The Work gates table and current forecast are published on GitHub; application-code publication remains separately controlled. Controller schedule metadata remains ACTIVE at the unchanged ten-minute cadence.

**Estimated remaining controller runs:** **23–39 bounded runs**, assuming tests remain healthy and owner reviews or external approvals do not add rework.

**Publication status:** the Work gates table, its per-gate outstanding work and estimates, and this 23–39 cross-gate forecast are current locally and published on GitHub `main`. Application-code and other documentation publication remain separately controlled.
...
## Current project state
    ...
    production_changes_authorised: refresh-portfolio-health deployment and the supplied CommSec holdings import were explicitly approved and completed; no further production mutation without exact owner approval
    owner_approval_recorded: Owner Review A approved; MYDASH-002 and MYDASH-003 complete
    next_action: Await non-secret owner evidence of the configured Twelve Data ASX add-on, completed exchange registration and official public-display redistribution rights. The reference-only public-cohort lookup is approved but must not run until all three are verified. Until then create no mapping, retrieve no price and persist no provider data. Consecutive failure count remains zero.
