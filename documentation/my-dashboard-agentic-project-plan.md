...
| MYDASH-007 | IN_PROGRESS | Deterministic return evaluator and feedback loop | Implement idempotent forward-return evaluation, benchmark/FX handling, drawdown, data-quality states, scheduled operational execution and AI-versus-user comparison. Independently reproduce calculations from source observations and verify no look-ahead or entry-time conflation. Pause for Owner Review C using real pilot decisions before final completion. | Immutable checkpoint selection and raw/simulated returns now include exact-time direct/inverse FX, exact-session optional benchmarks, raw-close maximum drawdown, source-bound evidence and fail-closed missing/ambiguous states. Next define the non-deployed operational evaluator path and expose honest AI-versus-user Decision Lab comparisons; whole-gate regression, independent audit and real-pilot Owner Review C remain. The migration stays unapplied. | 5–9 |
...
**Forecast updated:** 6 September 2026, 15:42 Australia/Perth

**Current position:** MYDASH-006 remains complete after independent `PASS_WITH_ADVICE`. MYDASH-007 is the sole active Producer gate. The evaluator now deterministically selects bounded checkpoints, calculates raw and fee/slippage BUY returns, resolves only exact-time canonical direct/inverse FX and exact-session optional benchmarks, and calculates signed maximum drawdown over the raw-close path. Missing or ambiguous evidence fails closed, every selected observation and derived result is source-hash-bound, and non-BUY actions remain observational. Focused 16/16 and repository 154/154 tests, TypeScript, palette and diff checks pass; the migration is intentionally unapplied.
