...
| MYDASH-007 | IN_PROGRESS | Deterministic return evaluator and feedback loop | Implement idempotent forward-return evaluation, benchmark/FX handling, drawdown, data-quality states, scheduled operational execution and AI-versus-user comparison. Independently reproduce calculations from source observations and verify no look-ahead or entry-time conflation. Pause for Owner Review C using real pilot decisions before final completion. | Decision Lab now reads validated immutable owner return snapshots, exposes each cutoff/quality/source identity and keeps non-ranked AI-signal and user-paper cohorts separate. Missing and observational outcomes never become zero. Whole-gate regression, independent audit and real-pilot Owner Review C remain; isolated database execution and the proposed 07:00 schedule stay unapplied. | 3–6 |
