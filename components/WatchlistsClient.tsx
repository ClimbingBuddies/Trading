...
function WatchlistWorkspace({ ownerId, embedded }: { ownerId: string; embedded: boolean }) {
    ...
    async function loadResearch() {
      ...
      const readPlans = async () => {
        for (let batch = 0; batch < researchIds.length; batch += 100) {
          for (let offset = 0; ; offset += 500) {
            const r = await client.from('personal_prediction_plans').select('id,instrument_id,assessment_id,horizon_sessions,published_at,entry_deadline,action,thesis,risks,source_cutoff,entry_rule,entry_delay_sessions,holding_sessions,timing_reason,model_identity').eq('owner_user_id', ownerId).in('instrument_id', researchIds.slice(batch, batch + 100)).order('published_at', { ascending: false }).order('id').range(offset, offset + 499).abortSignal(signal)
            if (r.error) throw r.error
  ...
  return <div className={[styles.shell, !embedded ? styles.standalone : ''].join(' ')}>
    ...
    <div className={styles.filters}><div className={styles.segment} role="group" aria-label="Filter watched shares"><button aria-pressed={!onlyBuy} onClick={() => setOnlyBuy(false)}>All watched shares</button><button aria-pressed={onlyBuy} onClick={() => setOnlyBuy(true)}>AI buy candidates</button></div><div className={styles.segment} role="group" aria-label="Plan horizon"><button aria-pressed={horizon === 5} onClick={() => setHorizon(5)}>Weekly</button><button aria-pressed={horizon === 20} onClick={() => setHorizon(20)}>Monthly</button></div><span className={styles.muted}>AI timing · {horizon}-session outlook</span><Link href="/my-dashboard?tab=decision-lab">View performance in Decision Lab →</Link></div>
