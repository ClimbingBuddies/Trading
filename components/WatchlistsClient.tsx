...
import { matchingPlan, olderAssessment, ratingAction, timingText } from '@/lib/watchlist-recommendations.mjs'
...
type Plan = { id: string; instrument_id: string; assessment_id: string; horizon_sessions: number; published_at: string; entry_deadline: string; action: string; thesis: string; risks: string; source_cutoff: string; entry_rule: string; entry_delay_sessions: number; holding_sessions: number | null; timing_reason: string | null; model_identity: string }
...
const actionLabel = { BUY: 'Buy candidate', HOLD: 'Hold', WAIT: 'Wait', SELL: 'Sell', REDUCE: 'Reduce', AVOID: 'Avoid for now', UNKNOWN: 'Not rated' }
...
function WatchlistWorkspace({ ownerId, embedded }: { ownerId: string; embedded: boolean }) {
    ...
    async function loadResearch() {
      ...
      const readPlans = async () => {
        for (let batch = 0; batch < researchIds.length; batch += 100) {
          for (let offset = 0; ; offset += 500) {
            const r = await client.from('personal_recommendation_views_v3').select('id,instrument_id,assessment_id,horizon_sessions,published_at,entry_deadline,action,thesis,risks,source_cutoff,entry_rule,entry_delay_sessions,holding_sessions,timing_reason,model_identity').eq('owner_user_id', ownerId).in('instrument_id', researchIds.slice(batch, batch + 100)).order('published_at', { ascending: false }).order('id').order('horizon_sessions').range(offset, offset + 499).abortSignal(signal)
            if (r.error) throw r.error
  ...
  const rows = activeItems.filter(i => !onlyBuy || (matchingPlan(plans, i.instrument_id, assessmentMap.get(i.instrument_id)?.assessment_id, horizon)?.action ?? ratingAction(assessmentMap.get(i.instrument_id)?.rating)) === 'BUY')
  ...
  return <div className={[styles.shell, !embedded ? styles.standalone : ''].join(' ')}>
    <Heading className={embedded ? styles.srOnly : undefined}>My watchlist &amp; recommendations</Heading>
    ...
    <details className={styles.method}><summary>How recommendations work</summary><p>AI views come from the latest saved scheduled assessment for each share. The assessment date is shown; views older than 72 hours are labelled. When a matching published plan exists, its AI decision takes priority and can differ between horizons; the original assessment remains in Details.</p><p>New calls record Buy, Wait, Hold, Sell or Reduce using fresh evidence. Later updates are added to the original Decision Lab journal. Weekly and monthly are performance checkpoints, not forced sell dates. A Buy is tested at a complete close after publication; a later Sell closes the paper position. Reduce is advisory only. Entry evidence expires after 14 days. Older timing records retain their original rules. A view appears only when it matches the displayed assessment. No orders are placed. Full timing rules and outcomes are in Decision Lab.</p><p>Research can predate tracking. It is never backdated into your performance record. Removing a share or list does not remove published plans.</p></details>
