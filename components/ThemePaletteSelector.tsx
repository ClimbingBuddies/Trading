...
const PALETTES = [
  { value: 'opportunity-blue', label: 'Opportunity Blue' },
  { value: 'midnight-blue', label: 'Midnight Blue' },
  { value: 'original-green', label: 'Original Green' },
  { value: 'aurora-slate', label: 'Aurora Slate' },
] as const
...
export default function ThemePaletteSelector() {
  const [palette, setPalette] = useState<PaletteId>('opportunity-blue')

  useEffect(() => {
    const saved = window.localStorage.getItem(STORAGE_KEY)
    const resolved: PaletteId = isPalette(saved) ? saved : 'opportunity-blue'
    setPalette(resolved)
